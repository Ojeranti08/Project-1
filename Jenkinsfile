        ...
        stage ('Run Container on Tomcat-server') {
            steps {
                script {
                    ...
                    // Build and run the new container with the unique name, mapping container port 8080 to host port 8081
                    def dockerRun = "sudo docker container run -dt --name ${containerName} -p 8081:8080 ojeranti08/javaapp:1.3.5"
